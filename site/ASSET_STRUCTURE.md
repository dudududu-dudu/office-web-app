...
- **本地开发**: 使用 `python run_server.py` 或 `docker compose up -d` 均无需特殊配置
- **Docker 部署**: `Dockerfile.site` 通过 `COPY ./site/ /usr/share/nginx/html/` 自动包含 assets 目录
- **CDN**: 可配置将 `assets/` 路径指向 CDN
